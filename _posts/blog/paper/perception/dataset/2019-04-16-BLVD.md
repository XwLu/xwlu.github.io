...
title="FDE=\frac{\sum_{i=1}^{N}\sum_{m=1}^{M}\sum_{t=t_{obs}+1}^{t_{obs}+t_{pred}}\sqrt{(x_{t}^{i}-\hat{x}_{t}^{i})^{2}+(y_{t}^{i}-\hat{y}_{t}^{i})^{2}}}{(N+M)t_{pred})}"
/>
...
title="FDE=\frac{\sum_{i=1}^{N}\sum_{m=1}^{M}\sqrt{(x_{pred}^{i}-\hat{x}_{pred}^{i})^{2}+(y_{pred}^{i}-\hat{y}_{pred}^{i})^{2}}}{(N+M)t_{pred})}"
/>

$ (x_i, y_i) $表示ground truth，$ (\hat x_i, \hat y_i) $表示预测值。N是轨迹数量，M是将轨迹平分为轨迹片段后的batches的数量。$ t_obs $是观测的帧数，$ t_pred $是预测的帧数。
