...
<a
href="https://www.codecogs.com/eqnedit.php?latex=ADE=\frac{\sum_{i=1}^{N}\sum_{m=1}^{M}\sum_{t=t_{obs}&plus;1}^{t_{obs}&plus;t_{pred}}\sqrt{(x_{t}^{i}-\hat{x}_{t}^{i})^{2}&plus;(y_{t}^{i}-\hat{y}_{t}^{i})^{2}}}{(N&plus;M)t_{pred})}"
target="_blank"><img
...
title="ADE=\frac{\sum_{i=1}^{N}\sum_{m=1}^{M}\sum_{t=t_{obs}+1}^{t_{obs}+t_{pred}}\sqrt{(x_{t}^{i}-\hat{x}_{t}^{i})^{2}+(y_{t}^{i}-\hat{y}_{t}^{i})^{2}}}{(N+M)t_{pred})}"
/></a>
